...
import { COMMAND_LIST } from "./command-list.constant";

const EXTENSION_NAME = "command";
let popup: any;
    ...
    return [
      Suggestion({
        ...
        items: ({ query }: { query: string }) => {
          const withFilteredCommands = COMMAND_LIST.map((commandList) => {
            return {
              ...commandList,
          ...
          const withoutEmptyCommandList = withFilteredCommands.filter(
            (commandList) => commandList.commands.length > 0,
          );

          const withEnabledSettings = withoutEmptyCommandList.map(
            (commandList) => {
              return {
                ...commandList,
                commands: commandList.commands.map((command) => {
                  return {
                    ...command,
                    isEnabled: true,
                  };
        ...
        render: () => {
          ...
          return {
            ...
            onKeyDown(props: SuggestionKeyDownProps) {
              console.log("command.ts::onKeyDown => component", component);
